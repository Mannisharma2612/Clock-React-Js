...
export default class App extends React.Component {
  ...
  _animate = () => {
    const scalesStaggerAnimations = this.state.scales.map(animated => {
    ...
    Animated.parallel([
      Animated.stagger(TICK_INTERVAL / this.state.scales.length, scalesStaggerAnimations),
  ...
  render() 
  {
    const {index, scales: [smallQuadranScale, mediumQuadranScale, bigQuadranScale, secondScale, minuteScale, hourScale]} = this.state;
    const interpolated = {
      inputRange: [0,360],
    ...
    const transformSeconds = {
      transform : [{ rotate: secondDegrees.interpolate(interpolated) }, { scale: secondScale}]
    };

    const rotateMinutes = Animated.divide(
      secondDegrees, new Animated.Value(60)
    );
    const transformMinutes = {
      transform : [{ rotate: rotateMinutes.interpolate(interpolated) }, { scale: minuteScale}]
    };
    ...
    const transformHours = {
      transform : [{ rotate: rotateHours.interpolate(interpolated) }, { scale: hourScale}]
    };
